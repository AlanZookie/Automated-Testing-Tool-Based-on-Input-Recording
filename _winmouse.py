...
import ctypes
import time
from ctypes import c_short, c_char, c_uint8, c_int32, c_int, c_uint, c_uint32, c_long, c_ulong, byref, Structure, CFUNCTYPE, POINTER, pointer, sizeof, Union
...
screenwidth = int(user32.GetSystemMetrics(SM_CXSCREEN))
screenhight = int(user32.GetSystemMetrics(SM_CYSCREEN))

def move_to(x, y, scale = 1):
    # shcore.SetProcessDpiAwareness(PROCESS_PER_MONITOR_DPI_AWARE)
    # user32.SetCursorPos(int(x), int(y))
    global screenwidth
    global screenhight
    posX = int(int(x) * 65536 / (screenwidth*scale)-1)
    posY = int(int(y) * 65536 / (screenhight*scale)-1)
    user32.mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE, posX, posY, 0, 0)
...
def move_global(xx, yy, x, y):
    # global screenwidth
    # global screenhight
    # dx = int((x * 65536 / screenwidth - 1) - (xx * 65536 / screenwidth-1) )
    # dy = int((y * 65536 / screenwidth - 1) - (yy * 65536 / screenwidth-1) )
    dx = int(x - xx)
    dy = int(y - yy)
    extra = ctypes.c_ulong(0)
    ii_ = DUMMYUNIONNAME()
    ii_.mi = MOUSEINPUT(dx, dy, 0, MOUSEEVENTF_MOVE, 0, pointer(extra))
    cmd = INPUT(c_ulong(0), ii_)
    user32.SendInput(1, pointer(cmd), sizeof(cmd))
